...
impl Chat {
    ...
    pub fn to_completion_input(&self) -> crate::util::llm::CompletionInput {
        let mut messages = Vec::<crate::util::llm::Message>::with_capacity(self.messages.len() + 1);
        messages.push(crate::util::llm::Message {
            role: crate::util::llm::Role::System,
            content: SYSTEM_PROMPT.to_string(),
        });
        for m in &self.messages {
            let role = match m.role {
                ChatMessageRole::User => crate::util::llm::Role::User,
                ChatMessageRole::Assistant => crate::util::llm::Role::Assistant,
            };
            messages.push(crate::util::llm::Message {
                role,
...
const SYSTEM_PROMPT: &str = "You are a helpful assistant for the user's project. \
Answer concisely. If you do not know the answer, say so honestly.";
...
mod tests {
    ...
    #[test]
    fn chat_add_message_rejects_oversize() {
        let mut chat = make_chat();
        let big = "x".repeat(MAX_MESSAGE_CONTENT_BYTES + 1);
        assert!(
            chat.add_message(ChatMessageRole::User, big).is_err(),
